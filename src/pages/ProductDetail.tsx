...
const ProductDetail: React.FC = () => {
  ...
  // WhatsApp Custom Message and Image attachment states
  const [customWaMsg, setCustomWaMsg] = React.useState('');
  const [waPhoto, setWaPhoto] = React.useState<File | null>(null);
  const [waPhotoName, setWaPhotoName] = React.useState('');
  const [waPhotoUploadStatus, setWaPhotoUploadStatus] = React.useState('');
  const [waUploading, setWaUploading] = React.useState(false);

  const handleWaPhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      setWaPhoto(file);
      setWaPhotoName(file.name);
      setWaPhotoUploadStatus('📎 Photo attachment added! Ready to be sent on WhatsApp.');
    }
  };

  const handleRemoveWaPhoto = () => {
    setWaPhoto(null);
    setWaPhotoName('');
    setWaPhotoUploadStatus('');
  };

  const handleWaSubmit = async () => {
    setWaUploading(true);
    let finalMsg = customWaMsg.trim();

    if (waPhoto) {
      try {
        setWaPhotoUploadStatus('📤 Uploading photo attachment to database server...');
        if (!supabase) {
          throw new Error('Database connection unavailable');
        }
        const fileExt = waPhoto.name.split('.').pop();
        const fileName = `${Date.now()}-${Math.random().toString(36).substring(2, 9)}.${fileExt}`;
        const filePath = `inquiry-attachments/${fileName}`;

        // Attempt upload to Supabase bucket 'product-images'
        const { error } = await supabase.storage
          .from('product-images')
          .upload(filePath, waPhoto);
        ...
        if (photoUrl) {
          ...
          setWaPhotoUploadStatus('✅ Photo successfully uploaded!');
        }
      } catch (err: any) {
        console.warn('Supabase storage upload failed:', err.message);
        setWaPhotoUploadStatus('💡 Photo selected. Please click the attachment clip inside WhatsApp to select and send the image directly.');
      }
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="lg:grid lg:grid-cols-2 lg:gap-10">
          ...
          <div className="mt-8 lg:mt-0 space-y-6">
            ...
            {/* Direct Showroom Inquiry via WhatsApp */}
            <div className="bg-primary/5 border border-primary/10 rounded-2xl p-4 space-y-3.5 mt-2">
              <span className="text-[11px] font-bold text-[#C8962E] uppercase tracking-wider block">Direct WhatsApp Inquiry Builder</span>
              
              <div>
                <label className="block text-[10px] font-bold text-dark/50 mb-1 uppercase tracking-wider">Custom Notes / Request details:</label>
                <textarea
                  value={customWaMsg}
                  onChange={e => setCustomWaMsg(e.target.value)}
                  placeholder="e.g. Please send details for 1000 sq ft, polished finish..."
                  className="w-full px-3 py-2 border border-border/80 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all resize-none bg-surface"
                  rows={2}
                />
              </div>

              <div>
                <label className="block text-[10px] font-bold text-dark/50 mb-1 uppercase tracking-wider">Attach Photo / Drawing (Optional):</label>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    id="wa-photo-upload"
                    className="hidden"
                    onChange={handleWaPhotoChange}
                  />
                  <label
                    htmlFor="wa-photo-upload"
                    className="flex items-center gap-1.5 px-3 py-1.5 border border-dashed border-border/80 rounded-xl text-xs font-bold text-dark/70 hover:border-primary hover:text-primary transition-all cursor-pointer bg-surface select-none"
                  >
                    📷 Choose File
                  </label>
                  {waPhotoName && (
                    <div className="flex items-center gap-1 bg-surface px-2.5 py-1 rounded-lg border border-border/45 text-xs text-dark/70 max-w-[180px]">
                      <span className="truncate">{waPhotoName}</span>
                      <button type="button" onClick={handleRemoveWaPhoto} className="text-red-500 font-extrabold hover:text-red-700 ml-1 text-sm">×</button>
                    </div>
                  )}
                </div>
                {waPhotoUploadStatus && (
                  <p className="text-[10px] text-primary/80 font-bold mt-1 leading-normal">{waPhotoUploadStatus}</p>
                )}
              </div>
              ...
              <button
                ...
                className="w-full btn-wa justify-center py-3.5 text-sm font-bold shadow-lg text-white cursor-pointer select-none"
                style={{ backgroundColor: '#25D366' }}
              >
                {waUploading ? (
                  <span className="flex items-center gap-1.5">
                    <svg className="animate-spin h-3.5 w-3.5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
